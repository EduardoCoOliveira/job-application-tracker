...
export interface CreateApplicationInput {
  ...
  modality: JobApplication['modality'];
  jobUrl: string;
  notes: string;
...
export class JobApplicationService {
  ...
  createApplication(input: CreateApplicationInput): JobApplication {
    const current = this.applicationsSubject.getValue();
    const nextId = (Math.max(...current.map((item) => Number(item.id)), 0) + 1)
      .toString();
    ...
    const newApplication: JobApplication = {
      ...
      appliedAt: this.formatDate(new Date()),
      nextAction: 'Follow up with recruiter and prepare talking points',
      updatedAt: 'Today',
      source: 'Manual entry',
      ...
      timeline: [
        {
          ...
          description:
            'Added manually through the Angular register form with current context and notes.',
        },
      ],
    };

    const updated = [newApplication, ...current];
    this.persistApplications(updated);
  ...
  private loadApplications(): JobApplication[] {
    ...
    try {
      ...
      return parsed.length > 0 ? parsed : seedApplications;
    } catch {
      return seedApplications;
    }
  }

  private persistApplications(applications: JobApplication[]): void {
  ...
  private buildSummary(input: CreateApplicationInput): string {
    return `${input.position.trim()} at ${input.company.trim()} saved from the front-end flow with notes ready for the next step.`;
  }
